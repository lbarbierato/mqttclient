 ...
 * @param Counter a timer class with the methods:
 */
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE = 100, int MAX_MESSAGE_HANDLERS = 5>
class Client
{
...
private:
    ...
    void cleanSession();
    int cycle(Counter& timer);
    int waitfor(int packet_type, Counter& timer);
    int keepalive();
    int publish(int len, Counter& timer, enum QoS qos);

    int decodePacket(int* value, int timeout);
    int readPacket(Counter& timer);
    int sendPacket(int length, Counter& timer);
    ...
    Counter last_sent, last_received;
...
template<class Network, class Counter, int a, int MAX_MESSAGE_HANDLERS>
void MQTT::Client<Network, Counter, a, MAX_MESSAGE_HANDLERS>::cleanSession() 
{
...
template<class Network, class Counter, int a, int MAX_MESSAGE_HANDLERS>
MQTT::Client<Network, Counter, a, MAX_MESSAGE_HANDLERS>::Client(Network& network, unsigned int command_timeout_ms)  : ipstack(network), packetid()
{
...
#if MQTTCLIENT_QOS2
template<class Network, class Counter, int a, int b>
bool MQTT::Client<Network, Counter, a, b>::isQoS2msgidFree(unsigned short id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
...
template<class Network, class Counter, int a, int b>
bool MQTT::Client<Network, Counter, a, b>::useQoS2msgid(unsigned short id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
...
template<class Network, class Counter, int a, int b>
void MQTT::Client<Network, Counter, a, b>::freeQoS2msgid(unsigned short id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
        if (incomingQoS2messages[i] == id)
...
template<class Network, class Counter, int a, int b>
int MQTT::Client<Network, Counter, a, b>::sendPacket(int length, Counter& timer)
{
    int rc = FAILURE,
        sent = 0;
...
template<class Network, class Counter, int a, int b>
int MQTT::Client<Network, Counter, a, b>::decodePacket(int* value, int timeout)
{
    unsigned char c;
    int multiplier = 1;
    int len = 0;
 ...
 */
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::readPacket(Counter& timer)
{
    int rc = FAILURE;
    MQTTHeader header = {0};
    int len = 0;
...
template<class Network, class Counter, int a, int b>
bool MQTT::Client<Network, Counter, a, b>::isTopicMatched(char* topicFilter, MQTTString& topicName)
{
...
template<class Network, class Counter, int a, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Counter, a, MAX_MESSAGE_HANDLERS>::deliverMessage(MQTTString& topicName, Message& message)
{
    int rc = FAILURE;
...
template<class Network, class Counter, int a, int b>
int MQTT::Client<Network, Counter, a, b>::yield(unsigned long timeout_ms)
{
    int rc = SUCCESS;
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::cycle(Counter& timer)
{
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::keepalive()
{
    int rc = FAILURE;
...
template<class Network, class Counter, int a, int b>
int MQTT::Client<Network, Counter, a, b>::waitfor(int packet_type, Counter& timer)
{
    int rc = FAILURE;
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::connect(MQTTPacket_connectData& options)
{
    Counter connect_timer(command_timeout_ms);
    int rc = FAILURE;
    int len = 0;
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::connect()
{
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, MAX_MESSAGE_HANDLERS>::subscribe(const char* topicFilter, enum QoS qos, messageHandler messageHandler)
{
    int rc = FAILURE;
    Counter timer(command_timeout_ms);
    int len = 0;
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, MAX_MESSAGE_HANDLERS>::unsubscribe(const char* topicFilter)
{
    int rc = FAILURE;
    Counter timer(command_timeout_ms);
    MQTTString topic = {(char*)topicFilter, {0, 0}};
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::publish(int len, Counter& timer, enum QoS qos)
{
    int rc;
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName, void* payload, size_t payloadlen, unsigned short& id, enum QoS qos, bool retained)
{
    int rc = FAILURE;
    Counter timer(command_timeout_ms);
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName, void* payload, size_t payloadlen, enum QoS qos, bool retained)
{
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName, Message& message)
{
...
template<class Network, class Counter, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Counter, MAX_MQTT_PACKET_SIZE, b>::disconnect()
{
    int rc = FAILURE;
    Counter timer(command_timeout_ms);            // we might wait for incomplete incoming publishes to complete
